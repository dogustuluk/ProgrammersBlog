...
namespace ProgrammersBlog.Mvc.Areas.Admin.Controllers
{
    ...
    public class CategoryController : BaseController
    {
        ...
        public async Task<IActionResult> Add(CategoryAddDto categoryAddDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.ResultStatus == ResultStatus.Success)
                {
                    ...
                    });  //categoryAddAjaxModel -> Json formatında string olarak gelir
        ...
        [Authorize(Roles = "SuperAdmin,Category.Update")]
        [HttpPost]
        public async Task<JsonResult> UndoDelete(int categoryId) //category index'teki data-id=@category.Id kısmından alıyor olucaz buradaki categoryId'yi
        {
            var result = await _categoryService.UndoDeleteAsync(categoryId, LoggedInUser.UserName);
            var undoDeletedCategory = JsonSerializer.Serialize(result.Data); //.Data ile -> geriye CategoryDto'yu göndermiş oluyoruz
            return Json(undoDeletedCategory);
        }
        [Authorize(Roles = "SuperAdmin,Category.Delete")]
        [HttpPost]
        public async Task<JsonResult> HardDelete(int categoryId) //category index'teki data-id=@category.Id kısmından alıyor olucaz buradaki categoryId'yi
        {
            var result = await _categoryService.HardDeleteAsync(categoryId);
            var deletedCategory = JsonSerializer.Serialize(result); //.Data ile -> geriye CategoryDto'yu göndermiş oluyoruz
            return Json(deletedCategory);
        }
    }
}
