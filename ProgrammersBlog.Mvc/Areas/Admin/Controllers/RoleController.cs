    ...
    public class RoleController : BaseController
    {
        ...
        public async Task<IActionResult> Assign(int userId)
        {
            ...
            foreach (var role in roles)
            {
                RoleAssignDto roleAssignDto = new RoleAssignDto
                {
                    RoleId = role.Id,
                    RoleName = role.Name,
                    HasRole = userRoles.Contains(role.Name)//kullanıcıda rolun olup olmadığının kontrolü. Var ise true döner ve partial view karşımıza geldiğinde kullanıcının rolü seçili gelir.
                };
        ...
        public async Task<IActionResult> Assign(UserRoleAssignDto userRoleAssignDto)
        {
            if (ModelState.IsValid)
            {
                ...
                await UserManager.UpdateSecurityStampAsync(user);//kullanıcının rolü değiştiğinde sistem dışına atılıp girmesini sağar.
